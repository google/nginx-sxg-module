...
static ngx_command_t ngx_http_sxg_commands[] = {
    ...
    {ngx_string("sxg_cert_path"),
     NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_cert_chain, NGX_HTTP_LOC_CONF_OFFSET,
...
static char* ngx_http_sxg_merge_loc_conf(ngx_conf_t* cf, void* parent,
  ...
  ngx_conf_merge_value(conf->enable, prev->enable, false);
  ngx_conf_merge_uint_value(conf->sxg_max_payload, prev->sxg_max_payload,
                            64 * 1024 * 1024);
  ...
  ngx_log_error(NGX_LOG_DEBUG, cf->log, 0,
                "nginx-sxg-module: merge conf for [%V]", &prev->certificate);

  conf->signers = sxg_empty_signer_list();
  if (conf->sxg_max_payload == NGX_CONF_UNSET_SIZE) {
    conf->sxg_max_payload = 64 * 1024 * 1024;  // Limited to 64 MB
  }
  ...
  EVP_PKEY* privkey = load_private_key((const char*)conf->certificate_key.data);
  if (privkey == NULL) {
    ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
  ...
  if (!sxg_add_ecdsa_signer("nginx", /*date=*/0, /*expires=*/0,
                            (const char*)conf->validity_url.data, privkey, cert,
                            (const char*)conf->cert_url.data, &conf->signers)) {
    ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                  "nginx-sxg-module: failed to allocate memory");
    return NGX_CONF_ERROR;
  }

  sxg_signer_t* const signer = &conf->signers.signers[0];
...
static char* ngx_conf_set_cert_chain(ngx_conf_t* cf, ngx_command_t* cmd,
                                     void* conf) {
  ngx_http_sxg_loc_conf_t* ssc = conf;
